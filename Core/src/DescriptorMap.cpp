...
static constexpr auto create_first_set_layout = [](auto &device,
  ...
  layout_info.bindingCount = static_cast<Core::u32>(bindings.size());
  layout_info.pBindings = bindings.data();

  Core::verify(vkCreateDescriptorSetLayout(device->get_device(), &layout_info,
...
DescriptorMap::DescriptorMap(const Device &dev,
  ...
  set_descriptor_set_layouts(shader);

  std::vector<VkDescriptorSetLayout> layouts;
  for (const auto _ : for_frame()) {
    layouts.push_back(descriptor_set_layouts.at(0));
    layouts.push_back(descriptor_set_layouts.at(1));
  }
...
auto DescriptorMap::set_descriptor_set_layouts(const Shader &shader) -> void {
  descriptor_set_layouts = shader.get_descriptor_set_layouts();
}
